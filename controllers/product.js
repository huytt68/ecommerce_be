...
const getProducts = asyncHandler(async (req, res) => {
	...
	if (queries?.size) {
		...
		const sizeQuery = sizeArr.map((el) => ({
			size: { $regex: el, $options: 'i' },
		}));
	...
	if (queries?.q) {
		...
		queryObject = {
			$or: [
				{ size: { $regex: queries.q, $options: 'i' } },
	...
	const qr = { ...sizeQueryObject, ...formatedQueries, ...queryObject };
	...
	// Exceute query
	// Số lượng sp thoả mãn điều kiện !== số lương sp trả về 1 lần gọi API
...
const deleteProduct = asyncHandler(async (req, res) => {
	...
	return res.status(200).json({
		...
		mes: deletedProduct ? 'Deleted' : 'Cannot update product',
	});
});
